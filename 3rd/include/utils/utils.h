...
//最简单效率最高写法
void static split(const string& s, vector<string>& tokens, char delim = ' ') {
  tokens.clear();
...
static string str_format(size_t& size_buf, const char* format, Args... args) {
  size_buf = std::snprintf(nullptr, 0, format, args...) + 1;
...
static string get_current_time(const string& str = "%Y-%m-%d %H:%M:%S") {
